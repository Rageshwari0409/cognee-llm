...
def _explain_context(message: str, ctx: dict, food_data: dict | None, mode: str) -> dict:
    """
...
def _get_fitness_context(message: str, ctx: dict) -> str:
    """
    ...
    if any(kw in lower for kw in _FITNESS_KEYWORDS["pre_workout"]):
    ...
    if any(kw in lower for kw in _FITNESS_KEYWORDS["post_workout"]):
    ...
    if any(kw in lower for kw in _FITNESS_KEYWORDS["protein"]):
    ...
    if any(kw in lower for kw in _FITNESS_KEYWORDS["hydration"]):
    ...
    if any(kw in lower for kw in _FITNESS_KEYWORDS["vegan_veg"]):
        vv     = ctx["vegan_veg"]
    ...
    if any(kw in lower for kw in _FITNESS_KEYWORDS["injury"]):
    ...
    # ── Fat loss data ────────────────────────────────────────────────────────
    if any(kw in lower for kw in _FITNESS_KEYWORDS["fat_loss"]):
        fl      = ctx["fat_loss"]
    ...
    if any(kw in lower for kw in _FITNESS_KEYWORDS["bulking"]):
    ...
    # ── Supplements ──────────────────────────────────────────────────────────
    if any(kw in lower for kw in _FITNESS_KEYWORDS["supplements"]):
    ...
    if any(kw in lower for kw in _FITNESS_KEYWORDS["sleep_recovery"]):
    ...
    if any(kw in lower for kw in _FITNESS_KEYWORDS["gut_health"]):
        gh     = ctx["gut_health"]
    ...
    # ── Meal prep ────────────────────────────────────────────────────────────
    if any(kw in lower for kw in _FITNESS_KEYWORDS["meal_prep"]):
        mp       = ctx["meal_prep"]
...
class DietCoach:
    ...
    def chat(
        ...
        # ── 2. Skill gate — reject out-of-scope messages cheaply ────────────
        ...
        ctx_used = _explain_context(message, ctx, food_data, mode_used)
